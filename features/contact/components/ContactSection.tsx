...
const ContactSection = ({ className, entranceTrigger, navEntrance = false }: ContactSectionProps): React.JSX.Element => {
  ...
  return (
    <section className={cn("min-h-dvh relative overflow-hidden pb-16 lg:pb-20", className)}>
      {/* CONTACT watermark */}
      <motion.span
            ...
            <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:gap-20">
              ...
              <div className="flex flex-col gap-6 lg:gap-8 flex-1">
                <motion.p
                  ...
                  className="text-[13px] sm:text-[15px] lg:text-[20px] text-text-primary/80 font-body max-w-[700px] leading-relaxed text-pretty"
                >
              ...
              <motion.div variants={itemVariants} className="flex flex-col gap-4 lg:gap-6 min-w-[200px] lg:min-w-[250px]">
                <p className="text-[11px] sm:text-xs lg:text-[16px] font-bold text-text-primary/60 font-heading uppercase tracking-widest">
                  Media Sosial
                </p>
                {socialLinks.map((link) => (
                  ...
                  >
                    <span className="text-[18px] sm:text-[22px] lg:text-[32px] font-bold text-text-primary font-heading group-hover:text-interaction transition-colors duration-200">
                      {link.name}
                    </span>
                    <svg
                      className="size-4 lg:size-5 text-text-primary/60 group-hover:text-interaction transition-colors duration-200"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="2"
